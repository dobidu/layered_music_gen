...
**R-X1 — Package skeleton.** Create `src/musicgen/` with `__init__.py`, `__main__.py`, `cli.py`. Add `pyproject.toml` with `hatchling` build backend, `requires-python >= 3.9`, CLI entry point `musicgen = "musicgen.cli:app"`, and `[project.optional-dependencies].dev = ["pytest>=8.0", "pytest-cov>=5.0", "pytest-xdist>=3.5"]`. Add `typer>=0.12` as a runtime dependency. **[Status: CLOSED 2026-04-18 by Plan 03-01.](/home/bidu/musicgen/.planning/phases/03-package-skeleton-sampler-generators-extraction/03-01-SUMMARY.md)** Override applied: `requires-python = ">=3.10"` (typer+hatchling pins rule out 3.9; see RESEARCH Risk #1). **DurationValidator relocation (`src/musicgen/duration_validator.py`) closed 2026-04-18 by [Plan 03-02](/home/bidu/musicgen/.planning/phases/03-package-skeleton-sampler-generators-extraction/03-02-SUMMARY.md) — the package now owns both scaffolding and the first relocated domain module.**

**R-X2 — Sampler.** Extract `sampler.py` with a `SongParams` dataclass and all `generate_random_*` + `generate_song_arrangement` functions. Functions take an explicit `rng: random.Random` parameter — no use of the module-level `random`. Unit-tested. **[Status: CLOSED 2026-04-18 by Plan 03-03.](/home/bidu/musicgen/.planning/phases/03-package-skeleton-sampler-generators-extraction/03-03-SUMMARY.md)** `src/musicgen/sampler.py` ships 9 symbols (SongParams frozen dataclass + SongParams.sample classmethod + 7 rng-aware free functions + validate_measures_dict). AST static guard (`tests/test_sampler.py::test_no_bare_random_in_sampler`) proves zero bare `random.*` in sampler.py. 33 seeded-determinism tests lock RNG draw order for Phase 5's golden baseline. music_gen.py is now a shim re-exporting all sampler symbols with `_rng = random.Random()` threaded through 6 call sites (D-08). Full suite: 309 -> 342 passing.

**R-X3 — Generators.** Extract `generators/{chord,melody,bassline,beat}.py`. Each takes `SongParams` + an injected `rng` + the relevant pattern files. Unit-tested. **[Status: CLOSED 2026-04-18 by Plan 03-04](/home/bidu/musicgen/.planning/phases/03-package-skeleton-sampler-generators-extraction/03-04-SUMMARY.md); phase-gate verified 2026-04-19 by [Plan 03-05](/home/bidu/musicgen/.planning/phases/03-package-skeleton-sampler-generators-extraction/03-05-SUMMARY.md).** All 4 generators shipped in `src/musicgen/generators/` with explicit `rng: random.Random` parameters. `tests/test_music21_isolation.py` added as permanent regression guard confirming music21 does not mutate global random (D-24). `tests/conftest.py` deleted (D-16) — `pyproject.toml pythonpath=["."]` carries repo root. Full Phase-3 gate: 371 tests passing, full-package AST scan reports zero bare `random.<method>` across `src/musicgen/**/*.py`, `python music_gen.py` smoke reaches `mix_and_save` (environmental failure at soundfont selection, not a Phase-3 regression).
