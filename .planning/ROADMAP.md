...
Plans:
- [ ] 01-01-importability-and-arrangement-fix-PLAN.md — Wrap top-level execution in `__main__` guard and fix the arrangement re-roll bug (PITFALLS P-A) by computing arrangement once in `create_song` and threading it into `mix_and_save`. (R-S1, R-S3)
- [ ] 01-02-pydub-gain-pan-fix-PLAN.md — Fix pydub gain/pan no-op (PITFALLS P-B) so `levels.json` actually affects output. (R-S4)
